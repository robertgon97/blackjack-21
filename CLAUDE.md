...
7. **Navegación principal:** la `BarraEstado` del juego solo muestra datos (banca, en juego,
...
8. **Avatar = emoji | URL:** el `avatar` puede ser un emoji o la URL de la foto de Google. Para pintarlo
   usa siempre `core/widgets/avatar.dart` (`Avatar` / `avatarEsUrl`); nunca `Text(avatar)` directo (se
   vería la URL como texto). No metas el avatar en `photoURL` de Auth (un emoji no es URL válida).
